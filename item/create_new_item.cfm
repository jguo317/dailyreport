...
<cfif isdefined('form.submit')>
	<cfif form.item_id eq 0>
		<cfquery name="insert" datasource="#application.datasource#">
			insert into item_status(item_key, item_status, item_summary, item_priority, item_comment, item_tester, item_reporter, item_frn_user_id, item_created)
			values ('#form.item_key#', #form.item_status#, '#form.item_summary#', #form.item_priority#, '#form.item_comment#', #session.user_id#, #session.user_id#, #session.user_id#, 1)
		</cfquery>
	<cfelse>
...
<cfif form.item_id neq 0>
	<cfquery name="getItem" datasource="#application.datasource#">
		select item_key, item_status,item_summary, item_priority, item_comment
